...
export const useProducts = (url: string, config: SWRConfiguration = {}) => {
    ...
    const sortByType = (products: IProduct[]) => {
        ...
        const tShirtBocaChomba = sortProductsByTerm(tShirtBocaHeat, 'boca_juniors_chomba')
        const tShirtBocabuzo = sortProductsByTerm(tShirtBocaChomba, 'boca_juniors_condivo_22_light_training_suit')
        const bocaTshirt_ = sortProductsByTerm(tShirtBocabuzo, 'boca_juniors_printed_t-shirt')
        const tShirtRiver = sortProductsByTerm(bocaTshirt_, 'river_plate_home_t-shirt_2023')
        ...
        const tshirtGameDay = sortProductsByTerm(tshirtArgentinaVieja_, 'adidas_game_day_shirt_argentina')
        const tshirtchamp_ = sortProductsByTerm(tshirtGameDay, 'argentine_champion_shirt_2022')
        const tshirtPrematch = sortProductsByTerm(tshirtchamp_, 'argentina_prematch_jersey')
        ...
        const tshirtQatar = sortProductsByTerm(tShirtsMessi, 'argentina_home_shirt_messi_+_final_patch_+_set_patch_qatar22')
