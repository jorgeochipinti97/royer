  ...
  const getDiscountPrice = (code_: string) => {
    data &&
      data.map((e) => {
        ...
        e.name.toUpperCase() == code_.toUpperCase() &&
          setPercentage(e.percentage);
        e.name.toUpperCase() == code_.toUpperCase() &&
          setDiscountPrice_(finalAmount);
        e.name.toUpperCase() == code_.toUpperCase() && setIsDiscount(true);
        e.name.toUpperCase() == code_.toUpperCase() &&
          Cookie.set("discountCode", e.name.toUpperCase());
      });
  };

  const getPercentaje = (): number => {
    const porcentaje = 10 * summaryValues.total;
    const porcentajeFinal = porcentaje / 100;
    const finalAmount = summaryValues.total - porcentajeFinal;
    return finalAmount;
  };
        ...
        isDiscount ? (
          <>
            ...
            >
              <Typography variant="subtitle1">
                {`Discount-Code: ${
                  router.asPath.indexOf("/summary") > -1
                    ? code.toUpperCase()
                    : (orderValues &&
                        orderValues.codigoDeDescuento?.toUpperCase()) ||
                      ""
                } `}{" "}
              </Typography>
            </Grid>
            ...
            >
              <Typography variant="h6">
                {router.asPath.indexOf("/summary") > -1
                  ? currency.format(discountPrice_)
                  : orderValues &&
                    currency.format(
                      orderValues.precioFinal || summaryValues.total
                    )}
              </Typography>
            </Grid>
          </>
